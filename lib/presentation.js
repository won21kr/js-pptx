...
Presentation.prototype.load = function (data, done) {
  var zip = new JSZip(data);
        console.log("############ " + zip.file("docProps/thumbnail.jpeg").asUint8Array().length)
  ...
  for (var key in content) {
    if (content.hasOwnProperty(key)) {
      ...
      if (ext == '.xml' || ext == '.rels') {
        ...
        console.log("BUFFERING " + key + " length="+content[key].length)
...
Presentation.prototype.getSlideCount = function() {
  return +this.content["docProps/app.xml"]["Properties"]["Slides"][0];
  //return Object.keys(this.content).filter(function(key) { return key.substr(0,17) === "/ppt/slides/slide"}).length;
}

Presentation.prototype.getSlide = function (name) {
  return new Slide(this.content['ppt/slides/' + name + '.xml'], this);
}
...
Presentation.prototype.addSlide = function (layoutName) {
  ...
  var maxId = 0;
  this.content["ppt/presentation.xml"]["p:presentation"]["p:sldIdLst"][0]["p:sldId"].forEach(function(ob) { if (+ob["$"]["id"] > maxId) maxId = +ob["$"]["id"]})
  ...
  var sldCount = +this.content["docProps/app.xml"]["Properties"]["Slides"][0]
  this.content["docProps/app.xml"]["Properties"]["Slides"][0] = sldCount + 1;
